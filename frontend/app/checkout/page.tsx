...
function CheckoutContent() {
    ...
    // B2B2C Payment Method State
    const [paymentMethod, setPaymentMethod] = useState<"razorpay" | "corporate">("razorpay");
    const [corporateBalance, setCorporateBalance] = useState(14500);
    ...
    const handlePayment = async () => {
        ...
        // Simulate Gateway or Corporate Wallet Deduction
        setTimeout(() => {
            ...
            <div className="container mx-auto px-4 py-8 lg:py-12 max-w-4xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-6">
                            <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-4">Select Payment Method</h2>
                            <div className="space-y-3">
                                <label className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'razorpay' ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-300 bg-white'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'razorpay' ? 'border-indigo-600' : 'border-slate-300'}`}>
                                            {paymentMethod === 'razorpay' && <div className="w-2.5 h-2.5 bg-indigo-600 rounded-full" />}
                                        </div>
                                        <div>
                                            <div className="font-bold text-slate-900">UPI / Credit Card</div>
                                            <div className="text-xs text-slate-500">Google Pay, PhonePe, Cards via Razorpay</div>
                                        </div>
                                    </div>
                                    <CreditCard className="w-5 h-5 text-slate-400" />
                                </label>

                                <label className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'corporate' ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-300 bg-white'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'corporate' ? 'border-indigo-600' : 'border-slate-300'}`}>
                                            {paymentMethod === 'corporate' && <div className="w-2.5 h-2.5 bg-indigo-600 rounded-full" />}
                                        </div>
                                        <div>
                                            <div className="font-bold text-slate-900">Acme Corp Wallet</div>
                                            <div className="text-xs text-slate-500">Balance: <span className="font-bold text-emerald-600">₹{corporateBalance.toLocaleString()}</span></div>
                                        </div>
                                    </div>
                    ...
                    <div>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-lg relative overflow-hidden">
                                ...
                                {isProcessing ? (
                                    ...
                                    <>
                                        ...
                                        <span className="relative z-10 flex items-center justify-center gap-2">
                                            Pay Securely <ArrowRight className="w-5 h-5 group-hover:translate-x-1.5 transition-transform" />
                                        </span>
                                    </>
                                )}
                            </Button>
                            ...
                            <div className="mt-8 flex items-center justify-center gap-3 opacity-50 hover:opacity-100 transition-opacity">
                                ...
                                <div className="text-[11px] font-bold font-mono tracking-widest text-slate-500 uppercase">100% Secure Checkout</div>
                            </div>
                        </div>
                    </div>
